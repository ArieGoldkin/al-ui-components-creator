...
class TestChatEndpoint:
    ...
    def test_successful_chat_request(self, client, mock_anthropic_client, sample_chat_messages):
        ...
        # The API returns the parsed Claude response directly (title, fields, code)
    ...
    def test_chat_request_no_content_type(self, client, sample_chat_messages):
        ...
        response = client.post('/api/chat', 
                              data=json.dumps({'messages': sample_chat_messages}))
        
        # Should still work as Flask can parse JSON
        assert response.status_code in [200, 400, 500]  # Depends on other factors

    @patch('app.client', None)
    def test_chat_request_no_anthropic_client(self, client, sample_chat_messages):
    ...
    def test_malformed_claude_response(self, client, sample_chat_messages):
        ...
        with patch('app.client') as mock_client:
            ...
            # The API returns 200 with fallback response when JSON parsing fails
            assert response.status_code == 200
            data = response.get_json()
    ...
    def test_cors_headers(self, client, sample_chat_messages):
        ...
        # This test verifies the endpoint is accessible for cross-origin requests
